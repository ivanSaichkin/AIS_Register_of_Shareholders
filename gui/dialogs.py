...
class OperationDialog(QDialog):
    """Диалог для добавления операции по лицевому счету"""
    ...
    def setup_ui(self):
        ...
        self.account_ids = {}
        for acc in accounts:
            display_text = f"{acc['account_number']} - {acc['shareholder_name']}"
        ...
        self.issue_ids = {}
        for issue in issues:
            display_text = f"{issue['registration_number']} - {issue['company_name']}"
    ...
    def save(self):
        ...
        if self.type_combo.currentText() == "Списание" and quantity > account['current_balance']:
            QMessageBox.warning(self, "Ошибка", f"Недостаточно акций на счете. Доступно: {account['current_balance']}")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Ошибка", f"Ошибка сохранения: {str(e)}")# gui/dialogs.py
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QFormLayout, QLineEdit,
                             QPushButton, QHBoxLayout, QComboBox, QDateEdit,
                             QTextEdit, QMessageBox, QCheckBox)
from PyQt6.QtCore import QDate
