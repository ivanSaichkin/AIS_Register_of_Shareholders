...
class AdminTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["ID", "Полное наименование", "Сокращенное", "ИНН", "ОГРН", 
    ...
    def setup_other_tabs(self):
        ...
        self.issue_search_input.textChanged.connect(self.search_issues)
        ...
        self.issues_table.setHorizontalHeaderLabels(
            ["ID", "Компания", "Рег. номер", "Дата", "Тип", 
             "Категория", "Кол-во", "Номинал"]
        )
        ...
        self.shareholders_table.setHorizontalHeaderLabels(
            ["ID", "Тип", "Наименование", "ИНН", "Паспорт/ОГРН", "Адрес", "Статус"]
        )
        ...
        self.accounts_table.setHorizontalHeaderLabels(
            ["ID", "Акционер", "Номер счета", "Дата открытия", 
    ...
    def load_data(self):
        ...
        for i, company in enumerate(companies):
            self.table.setItem(i, 0, QTableWidgetItem(str(company['company_id'])))
            self.table.setItem(i, 1, QTableWidgetItem(company['full_name']))
            self.table.setItem(i, 2, QTableWidgetItem(company['short_name']))
            self.table.setItem(i, 3, QTableWidgetItem(company['inn']))
            self.table.setItem(i, 4, QTableWidgetItem(company['ogrn']))
            self.table.setItem(i, 5, QTableWidgetItem(company['address']))
            self.table.setItem(i, 6, QTableWidgetItem(f"{company['authorized_capital']:,.2f}"))
            self.table.setItem(i, 7, QTableWidgetItem(company.get('city', '')))
        ...
        for i, issue in enumerate(issues):
            self.issues_table.setItem(i, 0, QTableWidgetItem(str(issue['issue_id'])))
            self.issues_table.setItem(i, 1, QTableWidgetItem(issue['company_name']))
            self.issues_table.setItem(i, 2, QTableWidgetItem(issue['registration_number']))
            self.issues_table.setItem(i, 3, QTableWidgetItem(str(issue['registration_date'])))
            self.issues_table.setItem(i, 4, QTableWidgetItem(issue['share_type']))
            self.issues_table.setItem(i, 5, QTableWidgetItem(issue.get('category_series', '')))
            self.issues_table.setItem(i, 6, QTableWidgetItem(str(issue['quantity'])))
            self.issues_table.setItem(i, 7, QTableWidgetItem(f"{issue['nominal_value']:,.2f}"))
        ...
        for i, sh in enumerate(shareholders):
            self.shareholders_table.setItem(i, 0, QTableWidgetItem(str(sh['shareholder_id'])))
            self.shareholders_table.setItem(i, 1, QTableWidgetItem(sh['type']))
            self.shareholders_table.setItem(i, 2, QTableWidgetItem(sh['name']))
            self.shareholders_table.setItem(i, 3, QTableWidgetItem(sh.get('inn', '')))
            passport = sh.get('passport_data', '') or sh.get('ogrn', '')
            self.shareholders_table.setItem(i, 4, QTableWidgetItem(passport))
            self.shareholders_table.setItem(i, 5, QTableWidgetItem(sh['address']))
            self.shareholders_table.setItem(i, 6, QTableWidgetItem(sh['status']))
    ...
    def search_issues(self):
        search_text = self.issue_search_input.text()
        if search_text:
            issue = self.parent.share_issue_model.get_by_registration_number(search_text)
            if issue:
                self.issues_table.setRowCount(1)
                self.issues_table.setItem(0, 0, QTableWidgetItem(str(issue['issue_id'])))
                self.issues_table.setItem(0, 1, QTableWidgetItem(issue['company_name']))
                self.issues_table.setItem(0, 2, QTableWidgetItem(issue['registration_number']))
                self.issues_table.setItem(0, 3, QTableWidgetItem(str(issue['registration_date'])))
                self.issues_table.setItem(0, 4, QTableWidgetItem(issue['share_type']))
                self.issues_table.setItem(0, 5, QTableWidgetItem(issue.get('category_series', '')))
                self.issues_table.setItem(0, 6, QTableWidgetItem(str(issue['quantity'])))
                self.issues_table.setItem(0, 7, QTableWidgetItem(f"{issue['nominal_value']:,.2f}"))
            else:
    ...
    def search_accounts(self):
        ...
        if search_text:
            ...
            if account:
                ...
                self.accounts_table.setItem(0, 1, QTableWidgetItem(account['shareholder_name']))
                self.accounts_table.setItem(0, 2, QTableWidgetItem(account['account_number']))
                self.accounts_table.setItem(0, 3, QTableWidgetItem(str(account['open_date'])))
                self.accounts_table.setItem(0, 4, QTableWidgetItem(str(account.get('close_date', '-'))))
                self.accounts_table.setItem(0, 5, QTableWidgetItem(str(account['current_balance'])))
                self.accounts_table.setItem(0, 6, QTableWidgetItem(account['status']))
            else:
    ...
    def edit_company(self):
        ...
        if selected >= 0:
            company_id = int(self.table.item(selected, 0).text())
    ...
    def edit_issue(self):
        selected = self.issues_table.currentRow()
        if selected >= 0:
            issue_id = int(self.issues_table.item(selected, 0).text())
    ...
    def edit_shareholder(self):
        selected = self.shareholders_table.currentRow()
        if selected >= 0:
            shareholder_id = int(self.shareholders_table.item(selected, 0).text())
    ...
    def city_report(self):
        ...
        if city:
            ...
            self.report_text.setColumnCount(5)
            self.report_text.setHorizontalHeaderLabels(
                ["ID", "Наименование", "ИНН", "Адрес", "Уставной капитал"]
            )
            for i, company in enumerate(companies):
                self.report_text.setItem(i, 0, QTableWidgetItem(str(company['company_id'])))
                self.report_text.setItem(i, 1, QTableWidgetItem(company['full_name']))
                self.report_text.setItem(i, 2, QTableWidgetItem(company['inn']))
                self.report_text.setItem(i, 3, QTableWidgetItem(company['address']))
                self.report_text.setItem(i, 4, QTableWidgetItem(f"{company['authorized_capital']:,.2f}"))
    
    def status_report(self):
        status = self.status_combo.text()
        if status:
            shareholders = self.parent.shareholder_model.get_by_status(status)
            self.report_text.setRowCount(len(shareholders))
            self.report_text.setColumnCount(5)
            self.report_text.setHorizontalHeaderLabels(
                ["ID", "Наименование", "Тип", "ИНН", "Статус"]
            )
            for i, sh in enumerate(shareholders):
                self.report_text.setItem(i, 0, QTableWidgetItem(str(sh['shareholder_id'])))
                self.report_text.setItem(i, 1, QTableWidgetItem(sh['name']))
                self.report_text.setItem(i, 2, QTableWidgetItem(sh['type']))
                self.report_text.setItem(i, 3, QTableWidgetItem(sh.get('inn', '')))
                self.report_text.setItem(i, 4, QTableWidgetItem(sh['status']))
